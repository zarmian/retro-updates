...
<div class="container mainwrapper margin-top">
  <div class="row">
    <div class="container">
      ...
      <form data-toggle="validator" role="form" method="post" class="registration-form" action="{{ url('/shift/update/'.$shift->id) }}" style="margin-top: 20px;" enctype="multipart/form-data">
          ...
          <div class="col-sm-12 col-md-7 col-lg-7 col-xs-12 col-sm-offset-2 col-md-offset-2 col-sm-offset-0">
            ...
            <div class="form_container">
              ...
              <div class="col-md-6 col-sm-6 col-lg-6 col-xs-6 form-group no-padding-left">
                ...
                <input type="text" name="title" id="title" class="form-control1" placeholder="@lang('admin/shift.title')*" required="required" value="{{ $shift->title }}" />
              </div>

              <div class="col-md-6 col-sm-6 col-lg-6 col-xs-6 form-group no-padding">
                <label for="status" class="input_label">@lang('admin/shift.status_label')</label>
                <select name="status" id="status" class="form-control1" required="required">
                  <option value="1" @if($shift->status == 1) selected="selected" @endif>Active</option>
                  <option value="0" @if($shift->status == 0) selected="selected" @endif>InActive</option>
                </select>
              </div>

              <div class="col-md-6 col-sm-6 col-lg-6 col-xs-6 form-group no-padding-left">
                <label for="start_time" class="input_label">@lang('admin/shift.start_time')</label>
                <input type="text" name="start_time" id="start_time" class="form-control1 timepicker" placeholder="@lang('admin/shift.start_time')*" required="required" value="{{ date('h:i A', strtotime($shift->start_time)) }} " />
              </div>

              <div class="col-md-6 col-sm-6 col-lg-6 col-xs-6 form-group no-padding">
                <label for="end_time" class="input_label">@lang('admin/shift.end_time')</label>
                <input type="text" name="end_time" id="end_time" class="form-control1 timepicker" placeholder="@lang('admin/shift.end_time')*" required="required" value="{{ date('h:i A', strtotime($shift->end_time)) }}" />
              </div>
              ...
              <div class="col-md-12 col-sm-12 col-lg-12 col-xs-12 form-group no-padding">
                ...
                <textarea name="description" id="description" cols="30" rows="10" class="form-control2" placeholder="@lang('admin/shift.description_label')">{{$shift->description}}</textarea>
              </div>
